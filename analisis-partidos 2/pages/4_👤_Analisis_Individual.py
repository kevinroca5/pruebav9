...
evo_cat = st.selectbox("Categoría", PLAYER_CAT_ORDER, format_func=lambda c: PLAYER_CAT_LABELS[c], key="indiv_evo_cat")
evo_metric_options = {l: c for k, c, l, cat, p in PLAYER_METRICS if cat == evo_cat and c in pdf_player.columns}
evo_selected = st.multiselect("Métricas a graficar", list(evo_metric_options.keys()),
                               default=list(evo_metric_options.keys())[:2], key="indiv_evo_metrics")
...
if len(pdf_player) < 2:
    ...
    fig = go.Figure()
    for label in evo_selected:
        col = evo_metric_options[label]
        fig.add_trace(go.Scatter(x=x_labels, y=pdf_player[col], mode="lines+markers", name=label))
    fig.update_layout(height=420, hovermode="x unified", title=f"Evolución de {selected_player}")
...
st.caption("Promedio del jugador en los partidos seleccionados, con su ranking frente a todos "
           "los jugadores que tengas cargados en la plataforma (el tamaño de esta comparativa "
           "crece según vayas subiendo más partidos).")
...
def build_player_pool():
    ...
    from player_common import list_player_match_files, PLAYER_MATCHES_DIR
    frames = []
...
for cat in PLAYER_CAT_ORDER:
    for key, col, label, c, is_pct in PLAYER_METRICS:
        ...
        if not pool_df.empty and col in pool_df.columns:
            ...
            if len(pool_vals) >= 2:
                rank = int((pool_vals >= val).sum()) if not is_pct or True else None
                n = len(pool_vals)
